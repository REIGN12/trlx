...
default_config = TRLConfig(
    train=TrainConfig(
        ...
        checkpoint_dir="/root/data/jingchu/trlx_ckps/checkpoints/ppo_hh_vicuna-7b-lr4e-6",
    ),
    model=ModelConfig(model_path="/root/data/model_weights/vicuna-7b-v1.1", num_layers_unfrozen=2),
    tokenizer=TokenizerConfig(tokenizer_path="/root/data/model_weights/vicuna-7b-v1.1", truncation_side="left"),
    optimizer=OptimizerConfig(name="adamw", kwargs=dict(lr=4e-6, betas=(0.9, 0.95), eps=1.0e-8, weight_decay=1.0e-6)),
    scheduler=SchedulerConfig(name="cosine_annealing", kwargs=dict(T_max=10000, eta_min=4e-6)),
    method=PPOConfig(
        name="PPOConfig",
        num_rollouts=64,
        chunk_size=16,
